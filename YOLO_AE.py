...
TB_COUNT = len([d for d in os.listdir(os.path.expanduser('./results/')) if 'coco_' in d]) + 1
PATH = os.path.expanduser('./results/') + 'coco_' + '_' + str(TB_COUNT)
os.makedirs(PATH)
print("=================== Directory " , PATH ,  " Created ")
# PATH = "./results/coco__4"

# class AutoEncoder(frontend.YOLO):
#
...
#                  max_box_per_image,
#                  anchors):
#         super().__init__(backend, input_size, labels, max_box_per_image, anchors)
...
#         }
#
#         train_generator = ToharGenerator(train_imgs, generator_config, norm=normalize,
#                                          jitter=False)  # outputs (input,input) rather than (input, ground truth)
#         valid_generator = ToharGenerator(valid_imgs, generator_config, norm=normalize,
#                                          jitter=False)  # outputs (input,input) rather than (input, ground truth)
...
#                                      period=1)
#
#         ae_checkpoint = ModelCheckpoint('AE_weights_coco.h5',
#                                         monitor='val_loss',
#                                         verbose=1,
#                                         save_best_only=True,
#                                         mode='min',
...
#         ############################################
#
#         self.model.fit_generator(generator=train_generator,  # train_batch #(input, input)
#                                  steps_per_epoch=1,  # len(train_batch)
#                                  epochs=1,  # 100
#                                  verbose=1,
...
def predict(model, image, img_name, path=""):
    """
    input_size = IMAGE_H
...
def evaluate(model, generator,
    ...
    for i in range(generator.size()):
        ...
        pred_boxes = predict(model, raw_image, img_name, path=save_path)
...
#todo: check what are reasonable ratios in the zero bias AE
SUP_NUM_IMAGES = 200
UNSUP_NUM_IMAGES = 2000
...
shuffle(train_imgs)
print("======== train_imgs are been shuffled")
...
#todo: should train AE on 80% or all unsupervised+supervised data. now it's just the unsup
# split the training set (supervised date) into train and validation 80%, 20% respectively:
ae_unsup = train_imgs[-UNSUP_NUM_IMAGES:]
ae_train = ae_unsup[:int(UNSUP_NUM_IMAGES*0.8)]
ae_val = ae_unsup[-int(UNSUP_NUM_IMAGES*0.2):]

tohar_train_batch = ToharGenerator(ae_train, generator_config, norm=normalize,
                                   jitter=False)  # outputs (input,input) rather than (input, ground truth)
tohar_valid_batch = ToharGenerator(ae_val, generator_config, norm=normalize,
                                   jitter=False)  # outputs (input,input) rather than (input, ground truth)

"""we evaluate the model on the validation set"""
tohar_eval_batch = ToharGenerator(eval_imgs, generator_config, norm=normalize, jitter=False,
...
autoencoder.fit_generator(generator=tohar_train_batch,  # train_batch #(input, input)
                          steps_per_epoch=len(tohar_train_batch),
                          epochs=100,
                          verbose=1,
                          validation_data=tohar_valid_batch, #todo can be removed?
...
# todo: I think that this mAP is not accurate at all.
# todo plus: why are there only 79 values? each index is the index of img from generator? can we add the name of the img? there are other mAP evaluators that I sent Vangelis
# todo: what do you try to compare? we also should try to train yolo on the supervised data (in addition)
...
"""evaluating on YOLO with training on small supervised datatset"""
model_t = model
