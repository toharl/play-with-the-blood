...
TEST=0
#there are 200 annotated samples. #todo -- add 1k images for pre-training
...
OBJ_THRESHOLD = 0.5  # 0.5  # confidence value consider as positive
NMS_THRESHOLD = 0.45  # 0.45 # IOU value threshold
...
# TB_COUNT = len([d for d in os.listdir(os.path.expanduser('./results_racoon/')) if 'cococo_' in d]) + 1
# PATH = os.path.expanduser('./results_racoon/') + 'cococo_' + '_' + str(TB_COUNT)
# os.makedirs(PATH)

PATH = "./results_racoon/cococo__5"
...
######## FULL YOLO ##########
# Layer 1
x = Conv2D(32, (3, 3), strides=(1, 1), padding='same', name='conv_1', use_bias=False, trainable=False)(input_image)
x = BatchNormalization(name='norm_1', trainable=False)(x)
x = LeakyReLU(alpha=0.1)(x)
encoded = MaxPooling2D(pool_size=(2, 2))(x)
...
aeyolo = Model(input_image, decoded)
aeyolo.summary()
aeyolo.compile(optimizer='adam', loss='mse')
...
def ae_predict(model, image, i, img_name, path=""):
    """
    input_size = IMAGE_H
    ...
    if i <= 100:
        ...
        if not os.path.exists(path):
            ...
            pass
            # print("Directory ", path, " already exists")
        plt.savefig(path+ "/out_" + img_name)
...
def evaluate(model, generator,
             ...
             max_detections=MAX_BOX_PER_IMAGE,
             save_path=None):
...
ae_train_batch = ToharGenerator(train, generator_config, norm=normalize, jitter=False) #AE trained on all of the train set (supervised and unsupervised)
ae_valid_batch = ToharGenerator(val, generator_config, norm=normalize, jitter=False)

eval_batch = BatchGenerator(eval, generator_config, norm=normalize, jitter=False, shuffle=False)
...
yae_checkpoint = ModelCheckpoint(PATH+'/yae_weights_coco.h5',
                                monitor='val_loss',
                                verbose=1,
                                save_best_only=True,
                                mode='min',
...
path(PATH+'/logs/WAE/')
path(PATH+'/logs/T/')
path(PATH+'/logs/aeyolo/')
ae_tb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/AE/')) if 'rac_' in log]) + 1
ae_tensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/AE/') + 'rac_' + '_' + str(ae_tb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)

tb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/WAE/')) if 'rac_' in log]) + 1
tensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/WAE/') + 'rac_' + '_' + str(tb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)

ttb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/T/')) if 'rac_' in log]) + 1
ttensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/T/') + 'rac_' + '_' + str(ttb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)

yae_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/aeyolo/')) if 'rac_' in log]) + 1
yaetensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/aeyolo/') + 'rac_' + '_' + str(yae_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)
...
#
# history = autoencoder.fit_generator(generator=ae_train_batch,  # train_batch #(input, input)
#                           steps_per_epoch=len(ae_train_batch),
#                           epochs=EPOCHS,
#                           verbose=1,
#                           validation_data=ae_valid_batch,
#                           validation_steps=len(ae_valid_batch),
#                           callbacks=[early_stop, ae_checkpoint, ae_tensorboard], #[early_stop, ae_checkpoint, tensorboard],
#                           max_queue_size=3)
#
...
history = aeyolo.fit_generator(generator=ae_train_batch,  # train_batch #(input, input)
                          steps_per_epoch=len(ae_train_batch),
                          epochs=EPOCHS,
                          verbose=1,
                          validation_data=ae_valid_batch,
                          validation_steps=len(ae_valid_batch),
                          callbacks=[early_stop, yae_checkpoint, yaetensorboard], #[early_stop, ae_checkpoint, tensorboard],
                          max_queue_size=3)
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig(path(PATH+"/yae")+"/yoloAE_plot.jpg")
loss = history.history['loss']
val_loss = history.history['val_loss']
l = np.array(loss)
v = np.array(val_loss)
...
#saves in_imagename and out_imagename to dir named vis
#todo: model AE -- the encoder build from original layer, pretrained and freezed the weights.
#todo:          -- the decoder is same arch as our AE. then, it is trained.
def visualise(generator, autoencoder):
    for i in range(generator.size()):
        raw_image = generator.load_image(i)
        ...
        # out = autoencoder.predict(raw_image)
        # # make the boxes and the labels
        ae_predict(autoencoder, raw_image, i, img_name, path=PATH+str('/vis'))
    return
# autoencoder.load_weights(PATH+'/AE_weights_coco.h5')
# visualise(eval_batch, autoencoder)

aeyolo.load_weights(PATH+'/yae_weights_coco.h5')
visualise(eval_batch, aeyolo)
...
AE = evaluate(model, eval_batch, save_path=PATH+"/W_AE")
print("AE:\n",AE)
...
NO_AE = evaluate(yolo, eval_batch, save_path=PATH+"/NO_AE")
print("NO_AE:\n", NO_AE)
...
params={'sup to unsup ratio:':SUP,
        ...
        "EPOCHS": EPOCHS,
        "sup:": len(sup), "unsup:": len(unsup), "validation:": len(val), "evaluation:": len(eval)}
...
print("Saved model info and outputs to:"+ str(PATH))
exit()
...
plt.savefig("./results_racoon/results_person/coco__2/figure_T")
print("\a")
