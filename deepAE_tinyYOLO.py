...
UNSUP_NUM_IMAGES = 50000 if not TEST else 100
...
path(PATH+'/logs/WAE/')
path(PATH+'/logs/T/')
ae_tb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/AE/')) if 'cococo_' in log]) + 1
ae_tensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/AE/') + 'cococo_' + '_' + str(ae_tb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)

tb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/WAE/')) if 'cococo_' in log]) + 1
tensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/WAE/') + 'cococo_' + '_' + str(tb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)

ttb_counter = len([log for log in os.listdir(os.path.expanduser(PATH+'/logs/T/')) if 'cococo_' in log]) + 1
ttensorboard = TensorBoard(log_dir=os.path.expanduser(PATH+'/logs/T/') + 'cococo_' + '_' + str(ttb_counter),
                          histogram_freq=0,
                          write_graph=True,
                          write_images=False)
...
print("AE:\n",AE[0])
print(np.average(list(AE[0].values())), AE[1], AE[2])
...
print("NO_AE:\n", NO_AE[0])
print(np.average(list(NO_AE[0].values())), NO_AE[1], NO_AE[2])
...
#
print("model w AE:")
print(np.average(list(AE[0].values())))
print("NO_AE:")
print(np.average(list(NO_AE[0].values())))
# print("YOLO:")
# print(np.average(list(YOLO.values())))
...
# # f.write(str(YOLO)+"\n")
f.write("AVG:")
f.write(str(np.average(list(AE[0].values())))+"\n")
f.write(str(np.average(list(NO_AE[0].values())))+"\n")
# # f.write(str(np.average(list(YOLO.values())))+"\n")
f.write("LOG:"+"\n")
f.write(str(params) )
f.write(str(generator_config) )
f.write("all coco detection, TINY YOLO, init with random ")
#
f.close()
